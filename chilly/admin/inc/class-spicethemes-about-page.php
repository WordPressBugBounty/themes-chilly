...
if (!class_exists('Chilly_About_Page')) {
	class Chilly_About_Page {
		...
		function __construct() {
			...
			/* load welcome screen */
			add_action( 'chilly_info_screen', array( $this, 'getting_started' ), 	    10 );
			add_action( 'chilly_info_screen', array( $this, 'github' ), 		            40 );
			add_action( 'chilly_info_screen', array( $this, 'welcome_free_pro' ), 				50 );
			add_action( 'chilly_info_screen', array( $this, 'recommended_actions' ), 				50 );
			}
	...
	public function scripts_for_customizer() {
		...
		public function welcome_admin_notice() {
			...
			<div class="updated notice is-dismissible">
				<p><?php echo sprintf( esc_html__("Chilly theme is installed. To take full advantage of the features this theme has to offer visit our %1\$s welcome page %2\$s", "chilly"), '<a href="' . esc_url( admin_url( 'themes.php?page=chilly-welcome' ) ) . '">', '</a>' ); ?></p>
				<p><a href="<?php echo esc_url( admin_url( 'themes.php?page=chilly-welcome' ) ); ?>" class="button" style="text-decoration: none;"><?php esc_html_e( 'Get started with Chilly theme', 'chilly' ); ?></a></p>
			</div>
			<?php
		}
...
function chilly_recommended_plugins_array($plugins){
		...
		$plugins[] = array(
			        'name' => esc_html__('WooCommerce', 'chilly'),
			        'slug' => 'woocommerce',
			        'desc' => esc_html__('To create a shop page you just need to install this plugin & activate It.', 'chilly'),
			    );

		$plugins[] = array(
					'name'     => esc_html__('Spice Post Slider', 'chilly'),
					'slug'     => 'spice-post-slider',
					'desc'     => esc_html__('To display the posts in beautiful slider with multiple options you just need to install & activate this plugin.', 'chilly'),
				);

		$plugins[] = array(
					'name'     => esc_html__('Spice Social Share', 'chilly'),
					'slug'     => 'spice-social-share',
					'desc'     => esc_html__('To add social share buttons to your posts you just need to install & activate this plugin.', 'chilly'),
				);

		$plugins[] = array(
					'name'     => esc_html__('Seo Optimized Images', 'chilly'),
		            'slug'     => 'seo-optimized-images',
		            'desc'     => esc_html__('It is recommended that you install & activate the Seo Optimized Images plugin to dynamically insert SEO Friendly alt attributes and title attributes to your Images.', 'chilly'),
				);
...
function Chilly_About_Page() {
	return Chilly_About_Page::get_instance();
}
global $chilly_about_page;
$chilly_about_page = Chilly_About_Page();
